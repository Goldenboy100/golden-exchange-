    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-6 relative overflow-hidden transition-colors">
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-primary rounded-[2rem] mb-6 shadow-2xl animate-in zoom-in duration-500">
            <TrendingUp size={44} />
          </div>
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight italic mb-2">{config.translations?.[config.language]?.login_title || config.appName}</h1>
          <p className="text-slate-500 dark:text-slate-500 font-bold uppercase tracking-[0.3em] text-[10px]">
            {isRegistering ? 'Join the Network' : 'Secure Access Terminal'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-10 rounded-[2.5rem] shadow-2xl border border-white/20 dark:border-white/5 space-y-6 animate-in fade-in-50 slide-in-from-bottom-10 duration-500">
          ...
          {isRegistering && (
            <div className="space-y-3 animate-in slide-in-from-left-10 fade-in duration-300">
              <label className="text-xs font-black text-slate-400 dark:text-slate-500 block mr-2 uppercase tracking-widest">ناو (Name)</label>
          ...
          <div className="text-center pt-2">
            ...
            >
              {isRegistering ? 'Already have an account? Login' : 'New User? Request Access'}
            </button>
          </div>
        </form>
      </div>
